...
class GFPCore6(GFPCore):
    ...
    def __init__(self,tifPath) -> None:
        ...
        self._imgcls = (self._filename.split('.')[-2]).split('-')[-1]
        self.xmlParse()
    ...
    @staticmethod
    def acquireGainBais(sensorType,year,imgcls,bId,satparams):
        """获取定标参数
        ...
        """
        if imgcls == 'MUX':
            _Gain = satparams["Parameter"]['GF6'][sensorType][year][imgcls]['gain'][bId-1]
            _Bias = satparams["Parameter"]['GF6'][sensorType][year][imgcls]['offset'][bId-1]
        elif imgcls == "PAN":
            _Gain = satparams["Parameter"]['GF6'][sensorType][year][imgcls]['gain'][bId-1]
            _Bias = satparams["Parameter"]['GF6'][sensorType][year][imgcls]['offset'][bId-1]
        return _Gain,_Bias
    ...
    def radiometricCalibration(self,outPath,satparams,*args, **kwargs):
        ...
        for m in range(1,_nbands+1):  
            ...
            i = 0
            j = 0
            _Gain,_Bias = self.acquireGainBais(self._sensortype,self._year,self._imgcls,bId,satparams)
    ...
    def atmospheriCorrection(self,outPath,satparams,*args, **kwargs):
        ...
        for m in range(1,_nbands+1):  
            ...
            _Gain,_Bias = self.acquireGainBais(self._sensortype,self._year,self._imgcls,bId,satparams)
            if _nbands == 1:
                SRF = satparams["SRF"][self._gftype][self._sensortype]['5']
                AtcCofa, AtcCofb, AtcCofc = self.acquire6sparams(self.__BandRange['5'][0],self.__BandRange['5'][1],SRF)
            else: 
...
def main():
    ...
    _tifpath = r'E:\GFDATA1\test\GF6_PMS_E98.6_N33.6_20200826_L1A1120056020\GF6_PMS_E98.6_N33.6_20200826_L1A1120056020-PAN.tiff'
    # GFPCore.initDatabase(SATPARAMS)
    gfpc = GFPCore6(_tifpath)
